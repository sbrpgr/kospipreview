...
const NOTICE_ITEMS: NoticeItem[] = [
  {
    title: "코스피프리뷰 개요 · 퀀트투자모델 개발을 위한 연구환경",
    blocks: [
      {
        kind: "paragraph",
        text: "코스피프리뷰는 다음 거래일 코스피 시초가를 연구하기 위해 만든 공개 연구환경입니다. 단순 시황 페이지가 아니라, 해외 ETF·환율·미국 지수·금리·원자재·야간선물 참고값을 하나의 파이프라인으로 엮어 실제 시초가와 반복 검증하는 퀀트투자모델 테스트베드에 가깝습니다.",
      },
      {
        kind: "paragraph",
      ...
      {
        kind: "paragraph",
        text: "핵심 포인트는 단순 숫자 제공이 아니라 구조적 설명력입니다. 어떤 신호를 코어로 쓰는지, 어떤 신호는 잔차 보정에만 쓰는지, 어떤 보정이 성능 개선에 기여했는지, 그리고 그 결과가 최근 실측 기록에서 어느 정도 검증됐는지를 함께 보여주는 것이 코스피프리뷰의 역할입니다.",
      },
    ],
  },
  {
    title: "현재 메인 예측모델 · EWY 프리장 브릿지 보정형 예측 엔진",
    blocks: [
      {
        kind: "paragraph",
        text: "현재 화면에 표시되는 메인 예측값은 EWY가 한국장 마감 시점에 거래되지 않는 공백을 보정하기 위해 미국 프리장 시작 직후 야간선물 브릿지를 1회 사용합니다. 그 이후 움직임은 EWY와 USD/KRW를 중심으로 한국장 마감 이후 누적된 해외 정보를 다시 계산해 다음 날 개장가를 추정합니다.",
      },
      {
        kind: "paragraph",
        text: "야간선물 단순환산은 비교용 기준선으로 분리되어 있습니다. 다만 EWY가 15:30에 직접 거래되지 않는 한계 때문에, 메인 모델과 EWY+환율환산은 프리장 시작 직후 2분 간격 5회 중 최신 야간선물 브릿지 표본으로 기준점을 맞춘 뒤 EWY와 환율 변화만 누적합니다.",
      },
      {
        kind: "formula",
      ...
      {
        kind: "paragraph",
        text: "왜 브릿지를 1회로 제한하는지가 중요합니다. 연구 목적은 단순히 야간선물을 따라가는 것이 아니라, EWY가 열리기 전의 공백만 보정하고 이후에는 EWY·환율·미국 지표가 누적하는 fair-value nowcast를 검증하는 데 있습니다. 다시 말해 이 플랫폼은 야간선물 복제기가 아니라, 야간선물 공백 보정 이후 독립 신호가 얼마나 설명력을 갖는지 실험하는 공개 연구환경입니다.",
      },
    ],
  },
  {
    ...
    blocks: [
      {
        kind: "paragraph",
        text: "코스피프리뷰는 장중 트레이딩 보조도구라기보다, 한국장 마감 이후 해외 정보가 누적되는 시간을 해석하기 위한 장전 연구도구에 가깝습니다. 따라서 가장 유용한 구간은 저녁부터 다음 날 아침 개장 직전까지이며, 낮 시간에는 '실시간 장중 판단 도구'로 쓰기보다 최근 기록과 모델 구조를 참고하는 용도로 보는 편이 적합합니다.",
      },
      {
        kind: "list",
        title: "추천 활용 방식",
        items: [
          "미국 프리장 시작 이후부터 09:00 구간에는 모델 예측값, 야간선물 단순환산, EWY·환율·반도체 지표를 함께 비교해 다음 날 개장 방향성을 점검합니다.",
  ...
  },
  {
    ...
    blocks: [
      {
        kind: "paragraph",
        text: "현재 코스피프리뷰는 단순한 야간선물 추종기가 아니라, 장마감 이후 해외 정보가 어떻게 다음 날 개장가에 반영되는지를 장기적으로 축적·검증하는 연구형 플랫폼입니다. 이런 유형의 모델은 하루나 일주일 성과보다, 데이터가 계속 쌓이면서 어떤 보정이 일관되게 설명력을 가지는지 확인할 때 비로소 강점이 드러납니다.",
      },
      {
        kind: "paragraph",
        text: "특히 EWY, 환율, 반도체, 금리, 원자재, 변동성 지표와 실제 시초가 사이의 관계를 밤마다 누적해 가면, 단순한 야간선물 지표를 넘어 훨씬 강한 설명력을 가진 독립형 예측모델로 발전할 가능성이 높습니다. 어떤 날에는 야간선물이 가장 직접적인 지표가 되지만, 또 어떤 날에는 야간선물만으로 설명되지 않는 공백 구간이 생기기 때문에 장기 누적 데이터는 이런 차이를 구분하는 데 매우 중요합니다.",
      },
      {
        kind: "paragraph",
        text: "따라서 이 플랫폼은 단기 적중률만 보는 서비스라기보다, 시간이 지날수록 더 강한 설명력을 갖춘 퀀트형 개장가 모델을 만들기 위한 공개 연구기반으로 이해하는 편이 맞습니다. 앞으로도 기록이 축적되고 검증 케이스가 늘어날수록 모델의 구조와 보정 방식은 더 정교해질 수 있으므로, 단순한 야간선물 참고 수준을 넘어서는 모델이 어떻게 자라나는지 지속적으로 관심을 가지고 지켜봐 주시면 좋겠습니다.",
      },
    ],
  },
  {
    title: "함께 발전시키는 플랫폼 · 주식시장 관련 연구·개발 문의를 환영합니다",
    blocks: [
      {
        kind: "paragraph",
        text: "코스피프리뷰는 완성된 고정형 서비스라기보다, 실제 데이터와 검증 기록이 쌓일수록 구조를 더 발전시켜 가는 공개 연구환경입니다. 따라서 단순히 개장가 예측에 한정하지 않고, 주식시장과 관련된 다양한 지표·모델·데이터 연구 아이디어가 있다면 문의사항을 통해 제안해 주셔도 좋습니다.",
      },
      {
        kind: "paragraph",
        text: "제안된 주제는 데이터 안정성, 수집 가능성, 지연 여부, 비용, 실제 설명력 가능성을 검토한 뒤 연구 파이프라인에 편입할 수 있는지 판단합니다. 단순히 지표를 많이 붙이는 것이 목적은 아니며, 시장 해석력을 높이거나 기존 공백 구간을 보완할 수 있는지, 혹은 별도의 유의미한 서비스·연구 축으로 확장할 수 있는지를 기준으로 검토합니다.",
      },
      {
        kind: "paragraph",
        text: "즉 이 플랫폼은 일방적으로 결과만 보여주는 페이지가 아니라, 사용자 제안과 실제 검증을 통해 점차 더 정교한 구조로 발전해 갈 수 있는 열린 연구환경입니다. 추가로 보고 싶은 지표, 비교해 보고 싶은 신호, 혹은 주식시장과 관련해 개발·연구가 필요하다고 생각하는 주제가 있다면 문의 페이지를 통해 알려 주시면 검토 후 개발 가능 여부를 살펴보겠습니다.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      {
        kind: "paragraph",
        text: "예측모델의 수준은 계산식만으로 결정되지 않습니다. 기준 시점 저장, 데이터 정제, 특징량 계산, 라이브 재연산, 최근 실측 기록 반영, 운영 상태 구분까지 하나의 파이프라인으로 안정적으로 이어져야 실제 검증이 가능합니다. 코스피프리뷰는 바로 이 운영 구조 전체를 모델 검증 범위에 포함합니다.",
      },
      {
        kind: "list",
        title: "운영 파이프라인 핵심",
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        kind: "paragraph",
        text: "바로 그 이유로 코스피프리뷰는 야간선물 단순환산을 비교 검증용으로 분리하고, 메인 예측은 EWY 기반 합성 KOSPI200 nowcast와 잔차 보정 알고리즘으로 계산합니다. 서비스의 본질은 야간선물 추종이 아니라, 야간선물 이후 공백 구간까지 설명할 수 있는 독립형 퀀트모델을 만드는 데 있습니다.",
      },
    ],
  },
  {
    ...
    blocks: [
      {
        kind: "paragraph",
        text: "코스피프리뷰는 연구·검증 목적의 정보 플랫폼이며, 금융투자상품에 대한 자문 서비스가 아닙니다. 본 사이트의 예측값, 단순환산치, 지표 카드, 해설, 기록 비교는 모두 참고 자료일 뿐이며 특정 종목, ETF, 선물, 옵션의 매수·매도를 권유하지 않습니다.",
      },
      {
        kind: "paragraph",
  ...
  return (
    <section className="noticeSection" aria-label="공지 및 안내">
      ...
      <p className="sectionSubtext noticeLead">
        본 플랫폼은 퀀트투자모델 개발과 검증을 위한 연구환경입니다. 메인 예측값은 야간선물 브릿지를 1회 보정으로 제한하는
